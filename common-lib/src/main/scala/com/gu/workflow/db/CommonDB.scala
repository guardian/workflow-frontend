...
object CommonDB {
  ...
  def updateContent(wc: WorkflowContent, revision: Long)(implicit session: Session): Int = {
      content
        ...
        .map(c =>
          (c.path, c.lastModified, c.lastModifiedBy, c.contentType, c.commentable, c.headline, c.trailtext, c.mainMedia, c.published, c.timePublished, c.revision, c.storyBundleId))
        .update((wc.path, wc.lastModified, wc.lastModifiedBy, wc.contentType, wc.commentable, wc.headline, wc.trailtext, wc.mainMedia, wc.published, wc.timePublished, Some(revision), wc.storyBundleId))
  }
